...
func (handle *updateRestaurantImpl) UpdateRestaurant(ctx context.Context, req *dtos.Restaurant) error {
	...
	if len(editEntities) > 1 {
		handle.contextlogger.WithContextInfo(ctx, enums.ContextKeyTraceID).Error("multiple edit entities error occur while updating restaurant, should only have one edit entity")
