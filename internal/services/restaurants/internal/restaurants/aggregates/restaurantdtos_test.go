...
func checkEditEntitiesWhileCreate(t *testing.T, editEntities *RestaurantEditEntities, dto *dtos.Restaurant) bool {
	...
	if editEntities.CreateEntities.Restaurants[0].Description != dto.Description {
		t.Errorf("expected Restaurant Description %v, got %v", dto.Description, editEntities.CreateEntities.Restaurants[0].Description)
		return false
	}
	// branches
	...
	for i, branch := range editEntities.CreateEntities.Branches {
		...
		if address.PostalCode != dto.Branches[i].Address.PostalCode {
			t.Errorf("expected Branch Address Postal Code %v, got %v", dto.Branches[i].Address.PostalCode, address.PostalCode)
			return false
		}
		if address.CountryCode.ToCountry() != dto.Branches[i].Address.Country {
			t.Errorf("expected Branch Address Country %v, got %v", dto.Branches[i].Address.Country, address.CountryCode.ToCountry())
			return false
		}
		// Price Range
		...
		if priceRange.MinPrice != dto.Branches[i].PriceRange.MinPrice {
			t.Errorf("expected Branch Price Range Min Price %v, got %v", dto.Branches[i].PriceRange.MinPrice, priceRange.MinPrice)
			return false
		}
		if priceRange.MaxPrice != dto.Branches[i].PriceRange.MaxPrice {
			t.Errorf("expected Branch Price Range Max Price %v, got %v", dto.Branches[i].PriceRange.MaxPrice, priceRange.MaxPrice)
			return false
		}
		// Category Relations
		categoryRelations := lo.UniqBy(lo.Filter(editEntities.CreateEntities.BranchCategoryRelations, func(relation entities.BranchCategoryRelation, _ int) bool {
		...
		if len(categoryRelations) != len(dto.Branches[i].Categories) {
			t.Errorf("expected Branch Category Relations %v, got %v", len(dto.Branches[i].Categories), len(categoryRelations))
			return false
		}
		...
		// Availables
		availableRelations := lo.Filter(editEntities.CreateEntities.Availables, func(available entities.Available, _ int) bool {
