...
type CreateBook aggregates.Book

type CreateBookHandler interface {
	CreateBook(ctx context.Context, command *CreateBook) error
}
...
func (handle *createBookImpl) CreateBook(ctx context.Context, command *CreateBook) error {
	if command == nil {
		return nil
	}
