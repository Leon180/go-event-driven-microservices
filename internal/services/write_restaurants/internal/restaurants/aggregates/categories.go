...
func (c *CategoryEntity) ToAggregate() *Category {
	a := Category(*c)
	return &a
}

type CategoryEntities entities.Categories

func (c CategoryEntities) ToAggregate() Categories {
	return lo.Map(c, func(category entities.Category, _ int) Category {
		categoryEntity := CategoryEntity(category)
		return *categoryEntity.ToAggregate()
	})
}
