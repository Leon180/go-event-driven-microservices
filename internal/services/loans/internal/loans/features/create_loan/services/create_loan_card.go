...
func (handle *createLoanImpl) CreateLoan(ctx context.Context, req *featuresdtos.CreateLoanRequest) error {
	...
	activeSwitch := true
	cards, err := handle.readLoanByMobileNumberAndActiveSwitch.ReadLoanByMobileNumberAndActiveSwitch(ctx, req.MobileNumber, &activeSwitch)
	if err != nil {
		return err
	}
	if len(cards) > 0 {
	...
	return handle.createLoanRepository.CreateLoan(ctx, &cardEntity)
}
